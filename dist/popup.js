...
async function refreshHealth() {
  ...
  const res = await send({ type: "HEALTH_CHECK" });
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("btn-add").addEventListener("click", async () => {
    ...
    const action = document.getElementById("memory-action").value.trim() || "noted";
    if (!text) {
  ...
  document.getElementById("btn-search").addEventListener("click", async () => {
    ...
    btn.disabled = true;
    const res = await send({ type: "SEARCH_MEMORY", query: q, limit: 8 });
    btn.disabled = false;
