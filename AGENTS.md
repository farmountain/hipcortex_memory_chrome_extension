...
| `npm run build` | works | `tsc` → `dist/`, then `scripts/build-content.js` bundles the one content-script entry, then `scripts/copy-assets.js` copies `public/*` and writes placeholder icons |
| `npm run watch` | partial | `tsc --watch` only — assets are **not** re-copied |
| `npm test` | works | `vitest run` across two projects (`node`, `jsdom`); 53 spec files / 821 tests, all passing on 2026-09-14. `passWithNoTests: false`, so a missing spec file fails the gate |
...
| `npm run package` | works | `npm run build && node scripts/package.js` — verified on Windows; the POSIX leg is unverified |
...
- **Icons are 1×1 placeholder PNGs** generated at build time — not design deliverables.
